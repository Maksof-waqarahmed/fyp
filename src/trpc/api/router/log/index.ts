...
export const logs = createTRPCRouter({
    ...
    getAllLogs: protectedProcedure.input(
        z.object({
            ...
            sortBy: z.enum(["checkedAt", "responseTime", "httpCode"]).optional().default("checkedAt"),
        ...
        try {
            ...
            // Filter by project (through endpoint)
            if (input?.projectId) {
            ...
            if (input?.dnsStatus) {
                whereClause.dnsStatus = input.dnsStatus;
            }
            ...
            if (input?.minResponseTime || input?.maxResponseTime) {
                whereClause.responseTime = {};
                if (input.minResponseTime) {
                    whereClause.responseTime.gte = input.minResponseTime;
                }
                if (input.maxResponseTime) {
                    whereClause.responseTime.lte = input.maxResponseTime;
                }
            }

            // Filter by SSL validity
            if (input?.sslValid !== undefined) {
                whereClause.sslValid = input.sslValid;
            }

            // Build order by clause
            const orderBy: any = {};
            orderBy[input?.sortBy ?? "checkedAt"] = input?.sortOrder ?? "desc";
